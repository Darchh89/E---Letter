...
class halaman_lihat_surat_dispensasi_siswa : AppCompatActivity() {

    private lateinit var rvSuratDispensasi: RecyclerView
    ...
    private var listSuratDispensasi = listOf<RiwayatSuratResponse>()
    private var currentKeyword = ""
    private var currentFilterStatus = "semua"

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.halaman_lihat_surat_dispensasi_siswa)

        findViewById<ImageView>(R.id.btn_back).setOnClickListener { finish() }

        rvSuratDispensasi = findViewById(R.id.rv_surat_dispensasi)
        rvSuratDispensasi.layoutManager = LinearLayoutManager(this)
    ...
    private fun muatDataSuratDispensasi() {
        ...
        RetrofitClient.instance.getHistorySurat(token).enqueue(object : Callback<List<RiwayatSuratResponse>> {
            override fun onResponse(call: Call<List<RiwayatSuratResponse>>, response: Response<List<RiwayatSuratResponse>>) {
                if (response.isSuccessful && response.body() != null) {
                    listSuratDispensasi = response.body()!!.filter { it.type_code == "dispensasi" }
                    terapkanFilterDanPencarian()
                    hitungStatistik(listSuratDispensasi)
                } else {
                    Toast.makeText(this@halaman_lihat_surat_dispensasi_siswa, "Gagal memuat data", Toast.LENGTH_SHORT).show()
                }
            }
            override fun onFailure(call: Call<List<RiwayatSuratResponse>>, t: Throwable) {
                Toast.makeText(this@halaman_lihat_surat_dispensasi_siswa, "Error koneksi", Toast.LENGTH_SHORT).show()
            }
        })
    }

    private fun terapkanFilterDanPencarian() {
        val listFiltered = listSuratDispensasi.filter { surat ->
        ...
        rvSuratDispensasi.adapter = RiwayatAdapter(listFiltered)
    }
