...
class halaman_detail_surat_izin_masuk : AppCompatActivity() {
    ...
    private val serverIp = "172.19.233.99"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Ambil ID dari Intent (Dikirim dari RiwayatAdapter)
        ...
        RetrofitClient.instance.getDetailSurat(token, requestId).enqueue(object : Callback<DetailSuratResponse> {
            override fun onResponse(call: Call<DetailSuratResponse>, response: Response<DetailSuratResponse>) {
                if (response.isSuccessful && response.body() != null) {
                    ...
                    Toast.makeText(this@halaman_detail_surat_izin_masuk, "Gagal mengambil detail", Toast.LENGTH_SHORT).show()
                }
            }
            override fun onFailure(call: Call<DetailSuratResponse>, t: Throwable) {
                Toast.makeText(this@halaman_detail_surat_izin_masuk, "Koneksi Error", Toast.LENGTH_SHORT).show()
            }
        })
    }
    ...
    private fun tampilkanDataKeLayar(data: DetailSuratResponse) {
        ...
        // 2. Dinamis Multi-Siswa
        val llContainerSiswa = findViewById<LinearLayout>(R.id.ll_container_siswa)
        llContainerSiswa.removeAllViews() // Bersihkan sisa layout jika ada

        for (siswa in data.students) {
            // Membuat Card Siswa secara dinamis
            val cardSiswa = MaterialCardView(this).apply {
                layoutParams = LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT).apply {
                    setMargins(0, 0, 0, 16)
                }
                setCardBackgroundColor(Color.parseColor("#F4E8FB"))
                radius = 24f // Corner radius
                cardElevation = 0f
            }
            ...
            val cardIkon = MaterialCardView(this).apply {
                ...
                setCardBackgroundColor(Color.parseColor("#EAD5F9"))
                radius = 45f
                cardElevation = 0f
            }
            val ikonUser = ImageView(this).apply {
                setImageResource(android.R.drawable.ic_menu_myplaces)
                setColorFilter(Color.parseColor("#A464DF"))
        ...
        // --- BAGIAN 3: LOGIKA STATUS & WARNA (Peringatan & TTD) ---
        ...
        when (data.status) {
            "approved" -> {
                ...
                tvStatusAtas.setTextColor(Color.parseColor("#4CAF50")) // Hijau
            ...
            "rejected" -> {
                ...
                tvStatusAtas.setTextColor(Color.parseColor("#F44336")) // Merah
                ...
                tvStatusAtas.setTextColor(Color.parseColor("#FF9800")) // Kuning
...
// --- BAGIAN 4: RENDER GAMBAR TANDA TANGAN (GLIDE) ---
        val ivTtdSiswa = findViewById<ImageView>(R.id.iv_ttd_siswa)

        // 1. Cari TTD Siswa (Bisa dari profil, atau dari tabel approvals)
        val ttdSiswaDariApproval = data.approvals.find { it.approver_role == "student" }?.signature_url
        val urlGambarSiswa = data.student_signature_url ?: ttdSiswaDariApproval

        if (!urlGambarSiswa.isNullOrEmpty()) {
            ivTtdSiswa.imageTintList = null
            val urlSiswa = "http://$serverIp:3000$urlGambarSiswa"
        ...
        // Teks Nama Pemohon di bawah TTD Siswa
        if (data.students.isNotEmpty()) {
            findViewById<TextView>(R.id.tv_ttd_nama_siswa).text = data.students[0].full_name
        }

        // 2. Cari TTD Guru (Approval yang role-nya BUKAN student)
        val approvalGuru = data.approvals.find { it.approver_role != "student" }

        if (data.status == "approved" && approvalGuru != null) {
            if (!approvalGuru.signature_url.isNullOrEmpty()) {
                ivTtdGuru.imageTintList = null
                val urlGuru = "http://$serverIp:3000${approvalGuru.signature_url}"

                Glide.with(this)
                    .load(urlGuru)
                    .into(ivTtdGuru)
            }
            // Ganti teks nama guru
            findViewById<TextView>(R.id.tv_ttd_nama_guru).text = approvalGuru.approver_name ?: "Petugas"
        }
    }
