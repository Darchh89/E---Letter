...
class halaman_daftar_siswa : AppCompatActivity() {
    ...
    // Siswa tidak punya etToken di layout
    ...
    private fun prosesDaftar() {
        ...
        // Eksekusi API
        val request = RegisterRequest(full_name = nama, email = email, password = password, token = "") // Token kosong untuk siswa
        ...
        RetrofitClient.instance.registerUser(request).enqueue(object : Callback<GeneralResponse> {
            override fun onResponse(call: Call<GeneralResponse>, response: Response<GeneralResponse>) {
                ...
                if (response.isSuccessful) {
                    ...
                    Toast.makeText(this@halaman_daftar_siswa, errorMsg, Toast.LENGTH_LONG).show()
                }
            }
