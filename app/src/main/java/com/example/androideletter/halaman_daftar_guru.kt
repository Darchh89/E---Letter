...
class halaman_daftar_guru : AppCompatActivity() {
    ...
    private fun prosesDaftar() {
        ...
        // Guru menggunakan role "teacher" dan mengirimkan input token
        val request = RegisterRequest(fullName = nama, email = email, password = password, role = "teacher", token = token)
        ...
        RetrofitClient.instance.registerUser(request).enqueue(object : Callback<RegisterResponse> {
            override fun onResponse(call: Call<RegisterResponse>, response: Response<RegisterResponse>) {
                ...
                if (response.isSuccessful && response.body()?.success == true) {
                    Toast.makeText(this@halaman_daftar_guru, "Pendaftaran Guru Berhasil!", Toast.LENGTH_SHORT).show()
                    startActivity(Intent(this@halaman_daftar_guru, halaman_masuk_user::class.java))
                    finish()
                } else {
                    Toast.makeText(this@halaman_daftar_guru, "Gagal: Token salah atau Email sudah terpakai", Toast.LENGTH_LONG).show()
                }
            }
